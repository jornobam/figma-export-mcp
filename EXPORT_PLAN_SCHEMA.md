...
План хранит отдельные `manifest` исходных изображений и `archiveManifest` для ZIP-режимов.
Каждый архив содержит неизменяемые `remotePath`, `groupKey`, `entries[]` с `itemId` исходного
изображения и точным именем записи ZIP. Оба manifest входят в digest. Preview показывает все
архивы в общей пагинации; коллизии изображение/архив и архив/архив должны блокировать
подтверждение. В `singleZip` и `zipPerGroup` изображения помечаются как `archive_entry`, поскольку
на Яндекс Диск как отдельные объекты отправляются только архивы.
