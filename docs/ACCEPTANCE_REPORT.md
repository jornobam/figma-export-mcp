...
- Read-only Figma REST client with normalized snapshots, complete selectors (including block and peer-dimension predicates), adaptive row/column geometry, batched PNG rendering, bounded retries and null-render errors.
- Unicode/cross-platform safe naming, deterministic ordering, regex/lookup variables and collision detection.
- Idempotent Yandex Disk directory creation, destination preflight before rendering, checksum-safe `skip_identical`, bounded uploads, timeout reconciliation, remote verification and post-verification cleanup.
